...
SimultaneousSubspaceGaussNewton<FltT>::SimultaneousSubspaceGaussNewton()
{
...
template <typename FltT>
bool SimultaneousSubspaceGaussNewton<FltT>::solveSubspace_(const cv::Mat_<FltT>& H, const cv::Mat_<FltT>& g, const size_t idx, const VecMax& init_update, VecMax& final_update) const
{
	bool ret = false;
	if (idx < OPTIMIZER_MAX_NUM_CAM)
	{
		ret = solveCameraPoseSubspace_(H, g, idx, init_update, final_update);
	}
	else
	{
		ret = solvePointSubspace_(H, g, idx - OPTIMIZER_MAX_NUM_CAM, init_update, final_update);
	}
	return ret;
}

template <typename FltT>
bool SimultaneousSubspaceGaussNewton<FltT>::solveCameraPoseSubspace_(const cv::Mat_<FltT> &H, const cv::Mat_<FltT> &g, const size_t idx, const VecMax &init_update, VecMax &final_update) const
{
...
template <typename FltT>
bool SimultaneousSubspaceGaussNewton<FltT>::solvePointSubspace_(const cv::Mat_<FltT>& H, const cv::Mat_<FltT>& g, const size_t idx, const VecMax& init_update, VecMax& final_update) const
{
	const i32 num_elem_cam = static_cast<i32>(OPTIMIZER_MAX_NUM_CAM * 6);
...
template class SimultaneousSubspaceGaussNewton<OptFltT>;